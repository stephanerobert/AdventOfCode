...
def is_valid(room_code):
    weighed_entry, sector_id, chksum, name = get_info(room_code)
    ...
    try:
        ...
        for j in chksum:
            i.remove(j)
            if len(i) == 0:
                i = weighed_entry[index]
                index = index+1

        return int(sector_id), name
    except Exception:
        return False, None

def get_info(room_code):
    occurence = {}
    entry = re.split('[-\[\]]', room_code)
    code = entry[:-3]
    sector_id, chksum = entry[-3:-1]
    for i in ''.join(code):
        value = occurence.get(i, 0)
        occurence[i] = value+1

    v = {}
    for key in sorted(occurence, key=occurence.__getitem__, reverse=True):
        v.setdefault(occurence[key], []).append(key)

    return [value for (key, value) in sorted(v.items(), reverse=True)], sector_id, chksum, code

def find_real_name(enc_name, code):
    decrypted_name = ''
    for i in enc_name:
        if i == '-':
            decrypted_name = decrypted_name + ' '
        else:
            enc_lettre_idx = string.ascii_lowercase.index(i)
            decrypted_name = decrypted_name + string.ascii_lowercase[(enc_lettre_idx+code)%26]
    return decrypted_name

if _run_tests:
    assert(is_valid('not-a-real-room-404[oarel]')[0] == 404)
    assert(is_valid('totally-real-room-200[decoy]')[0] == False)
    assert(find_real_name('qzmt-zixmtkozy-ivhz', 343) == 'very encrypted name')


room_codes='''bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
if __name__ == '__main__':
    total =0
    for i in room_codes.splitlines():
        code, name = is_valid(i)
        if code:
            total = total + code
            if 'northpole' in find_real_name('-'.join(name), code):
                print(code)
    print(total)
